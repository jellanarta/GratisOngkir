...
namespace Rule.Web.WebUserControl.GenericLookup
{
    public partial class LookupBaseSimple : System.Web.UI.UserControl
    {
        ...
        private Panel divGrid
        {
            get
            {
                return umd.FindControl("divGrid") as Panel;
            }
        }
        ...
        #region SHOW DATA
        private DataTable getData(bool isAutoComplete, int currentPage = 1, bool includeCount = false, bool includeData = true)
        {
            ...
            {
                pagingSpec.IncludeCount = includeCount;
                pagingSpec.RowPerPage = PageSize;
            ...
            int checkCount = pagingResult.Data.Rows.Count;
            foreach (DataRow dr in pagingResult.Data.Rows)
            {
                string checkResult = dr["SupplBranchName"].ToString();
            }
        ...
        {
            string scriptTemplate = MapperName + "({0})";
        ...
        protected void gvList_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowIndex > -1)
            {
                ...
                {
                    hpSelect.NavigateUrl = string.Format("javascript:{0};overlay('{1}');__doPostBack()",
                        dataSource.Rows[e.Row.RowIndex][selectColumn], ModalDialogName);
                }
        ...
        #region PAGING
        private void SearchClick(int currentPage = 1, bool includeCount = false, bool includeData = true)
        {
            DataTable dt = getData(false, currentPage, includeCount, includeData);
            if (includeData)
            {
